...
def main():
    try:
        ...
        for i in range(1):
            ...
            At present, there may be errors in the values of certain fields in  << API_FORMATTING >>.
            ...
            The format of each field in API is shown as << Field Type >>.
            ...
            IMPORTANT: << Field Type >> Only provide type explanations for each field in << API_FORMATTING >> field. During the filling process, you must refer to the << Field Type >>.
            ...
            command = "The original command is: " + translated_commands[0]
            command += "and users may wish to implement the following functions:"
            ...
            prompt = PromptTemplate(
                input_variables=["original_command", "json_output", "target_key_formatting"],
            ...
            url = "http://localhost:3000/api/details?path=" + target_api_names[0] + "&method=post"
            response = requests.get(url)

            if response.status_code == 200:
                # 假设 API 返回的 JSON 中包含 'target_key' 字段
                target_key_formatting = response.json()

            json_obj = set_values_to_null(json_obj)
            response = llm_chain.run(
                original_command=command,
                json_output=json_obj,
                target_key_formatting = target_key_formatting
            )
            try:
                # 获取 "</think>" 后的部分
