...
class API_target_selector:
    ...
    def main(self):
        try:
            # 加载 CSV 文件
            loader = CSVLoader(file_path="./tmp/filtered_results.csv", encoding="utf-8")
            docs = loader.load()
            ...
            # Chroma 索引的持久化存储路径
            persist_directory = "./tmp/filtered_tmp_api_paths_english"
            vector_store = None
            ...
            query = self.query_template.format(original_command=command, json_output=self.json_output)
            ...
            if match:
                ...
                with open('./tmp/target_api.csv', 'w', newline='', encoding='utf-8') as csvfile:
