...
class API_target_show:
    # 读取 Swagger API 文档
    def read_swagger_file(self, file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            return file.read()


    # 查找指定接口的地址
    def find_endpoint(self, markdown_content, target_endpoint):
    ...
    def convert_to_json(self, found_endpoints):
        # 转换为 JSON 格式字符串
        json_obj = json.loads(found_endpoints)
        return json_obj


    def print_json_keys(self, json_obj, parent_key=''):
        if isinstance(json_obj, dict):
            for key, value in json_obj.items():
                ...
                self.print_json_keys(value, new_key)  # 递归调用处理子字段
            ...
            for index, item in enumerate(json_obj):
                ...
                if isinstance(item, (dict, list)):  # 检查是否有儿子key
                    print(f"    {new_key} has children.")  # 有儿子key
                self.print_json_keys(item, new_key)

    def write_json_keys_to_csv(self, json_obj, parent_key='', writer=None):
        if isinstance(json_obj, dict):
            for key, value in json_obj.items():
                new_key = f"{parent_key}.{key}" if parent_key else key
                has_children = 'Yes' if isinstance(value, (dict, list)) else 'No'  # 判断是否有儿子
                writer.writerow([new_key, has_children])  # 写入 CSV
                self.write_json_keys_to_csv(value, new_key, writer)  # 递归调用处理子字段
        elif isinstance(json_obj, list):
            for index, item in enumerate(json_obj):
                new_key = f"{parent_key}[{index}]"
                has_children = 'Yes' if isinstance(item, (dict, list)) else 'No'  # 判断是否有儿子
                writer.writerow([new_key, has_children])  # 写入 CSV
                self.write_json_keys_to_csv(item, new_key, writer)
    ...
    def main(self):
        ...
        # 读取 Swagger 文件内容
        markdown_content = self.read_swagger_file(swagger_file)

        # 查找指定接口的地址
        found_endpoints = self.find_endpoint(markdown_content, target_endpoint)

        json_obj = self.convert_to_json(found_endpoints)
        # 写入到文件
        ...
        # 打开一个 CSV 文件并写入数据
        with open('./tmp/json_keys.csv', mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(['Field Name', 'Has Children'])  # 写入表头
            self.write_json_keys_to_csv(json_obj, writer=writer)

        print("CSV file 'json_keys.csv' has been created.")

        self.print_json_keys(json_obj)
